...
function buildSlotPrompt({
  ...
  const keywordLine = String(keyword || '').trim()
    ? `Focus keyword / prompt: "${String(keyword).trim()}"
Keyword rules:
- Every comment must naturally relate to this keyword/prompt while still sounding like a real YouTube viewer reacting to THIS video.
- Weave the keyword idea into the comment (mention it, react to it, ask about it, or connect the video to it) — do not paste the keyword awkwardly.
- Do not ignore the video; comments must still reference the video topic.`
    : '';
...
function fallbackTextForSlot(slot, videoTitle = 'this video', index = 0, keyword = '') {
  ...
  const focus = String(keyword || '').trim().slice(0, 60);
  const topic = focus || title;
  const variants = {
    english: [
      `The part about ${topic} really stood out to me`,
      `ngl this changed how I think about ${topic}`,
      `Been looking for something like this — solid breakdown on ${topic}`,
      `Lowkey one of the better takes I've seen on ${topic}`,
      `Wait the ending actually made me rethink ${topic}`,
    ],
    khasi: [
      `Ka jingbatai ha kane ka video ka long kaba shisha`,
      `Nga kwah ban pyndam ia ka jinglong kaba aiñ ${topic}`,
      `Kane ka long kaba sngewbha — kam ju ju sngew ia kane ha YouTube`,
      `Ka jingthmu kaba aiñ ${topic} ka long kaba pynshai shisha`,
      `Nga pyndep ba phi la ai ka jingtip kaba bha ha kane ka video`,
    ],
    hindi: [
      `${topic} pe jo point uthaya wo actually helpful tha`,
      `Yaar honestly is video ne ${topic} pe perspective change kar diya`,
    ...
    pnar: [
      ...
      `Ka jingthmu kaba aiñ ${topic} ka long kaba pynshai`,
      `Nga pyndep ba phi la ai ka jingtip kaba bha`,
    ],
    garo: [
      `Ia video-ni explanation-o nama`,
      `${topic} baksa ia video-o dakchakani nama`,
      `Ia video-o dakchakani ba nama`,
      `${topic}-ko nina ia video-o dakchakna`,
      `Ia channel-o first time nina — nama`,
    ],
  };
...
function processGeneratedComments(items, slots, spellingErrorRate = 0, videoTitle = 'this video', keyword = '') {
  return slots.map((slot, index) => {
    ...
    if (!text) {
      text = fallbackTextForSlot(slot, videoTitle, index, keyword);
    }
