const { Types } = require('../db/objectId');
  ...
  try {
    ...
    if (connectedProfile) {
      ...
      if (!isConnected) {
        ...
        // Reconstruct activeConnections entry if missing (e.g. after server restart)
        if (!openvpnService.activeConnections.has(connectedProfile._id.toString())) {
          openvpnService.activeConnections.set(connectedProfile._id.toString(), {
            status: 'connected',
            originalFileName: connectedProfile.originalFileName || connectedProfile.configFileName || 'unknown',
