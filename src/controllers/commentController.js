...
/**
 * Human-like gap between YouTube comments.
 * Picks from 30s / 40s / 50s / 1m / 2m / 3m.
 */
const COMMENT_GAP_SECONDS = [30, 40, 50, 60, 120, 180];

const pickCommentGapMs = () => {
  const seconds = COMMENT_GAP_SECONDS[Math.floor(Math.random() * COMMENT_GAP_SECONDS.length)];
  return seconds * 1000;
};
...
const waitCommentGap = async (label = 'next comment') => {
  const ms = pickCommentGapMs();
  console.log(`[postComments] Waiting ${formatGapLabel(ms)} before ${label}...`);
...
const postCommentsOnVideo = async (req, res, next) => {
  try {
    ...
    /** @type {{ accountId: string, comment: string }[]} */
    let jobs = [];

    if (Array.isArray(commentJobs) && commentJobs.length > 0) {
      jobs = commentJobs
        .map((job) => ({
          accountId: String(job.accountId || job.account_id || ''),
          comment: String(job.comment || job.text || '').trim(),
        }))
        .filter((job) => job.accountId && job.comment);
    } else if (accountComments && typeof accountComments === 'object') {
      for (const [accId, commentText] of Object.entries(accountComments)) {
        if (commentText && String(commentText).trim() !== '') {
          jobs.push({ accountId: String(accId), comment: String(commentText).trim() });
        }
      }
      ...
      for (const acc of allAccounts) {
        ...
        jobs.push({
          ...
          comment: String(commentPool.shift()).trim(),
        });
      }
    }
    ...
    const accountIds = [...new Set(jobs.map((job) => job.accountId))];
    ...
    for (let i = 0; i < jobs.length; i++) {
      ...
      try {
        ...
        const successResult = {
          ...
          email: account.email,
          name: account.name,
          youtubeChannel: account.youtubeChannel || posted.youtubeChannel || '',
          youtubeChannelTitle: account.youtubeChannelTitle || posted.youtubeChannelTitle || '',
          comment: commentText,
          status: 'success',
...
async function fetchVideoDetails(videoId) {
  ...
  // Fallback when YOUTUBE_API_KEY is not set (oEmbed — title only)
  console.warn("[YouTube] YOUTUBE_API_KEY missing — using oEmbed fallback for video metadata.");
  ...
  if (!oembedRes.ok) {
    throw new Error(
      "YouTube API key is missing. Add YOUTUBE_API_KEY to .env (Google Cloud → APIs → YouTube Data API v3 → Credentials)."
    );
  }
...
async function generateComments({
  ...
  if (Array.isArray(slots) && slots.length > 0) {
    return generateCommentsWithSlots({
      ...
      keyword: generationFilters?.keyword || filters?.keyword || '',
    });
  }
  ...
  if (generationFilters) {
    ...
    return generateCommentsWithSlots({
      ...
      keyword: generationFilters.keyword || '',
    });
  }
...
const regenerateCommentFromUrl = async (req, res, next) => {
  try {
    ...
    const generated = await generateCommentsWithSlots({
      ...
      keyword: normalizedFilters?.keyword || filters?.keyword || '',
    });
