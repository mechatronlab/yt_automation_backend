  ...
  try {
    ...
    if (process.env.FIVESIM_API_KEY) {
      try {
        ...
        if (response.ok) {
          ...
          fivesimBalance = Number(profile.balance ?? 0);
        } else {
          fivesimError = `5SIM profile HTTP ${response.status}`;
        }
      } catch (err) {
        fivesimError = err.message || 'Could not reach 5SIM';
      }
    } else {
      fivesimError = 'FIVESIM_API_KEY missing in .env';
    }
    ...
    res.status(200).json({
      ...
      fivesim: {
        ...
        error: fivesimError,
        ready: fivesimBalance != null && fivesimBalance > 0.05,
      },
      poolSizes: {
